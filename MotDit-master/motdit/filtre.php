    ...
    <section id="contentSection">
        ...
        <div class="col-md-8">
            <div class="single_post_content">
                <?php
                $compteur = 1;
                $infosArticlesCategorie = getInfosArticleCategorie($_GET['categorieID']);

                foreach ($infosArticlesCategorie as $informationCategorie) :
                    if($compteur == 1) {?>
                <h2><span><?php echo($informationCategorie['nom']) ?></span></h2>
                <?php } $compteur++ ?>
                <?php endforeach; ?>
                <?php
                if(!empty($informationCategorie['nom'])) {
                $infosArticlesCategorie = getInfosArticleCategorie($_GET['categorieID']);
                foreach ($infosArticlesCategorie as $informationCategorie) :
                    ?>
                <div class="media">
                        ...
                        <li>
                            <article style="padding-bottom: 20px; padding-top: 10px; border-bottom: dotted 5px #800049;">
                                <div class="media-left">
                                    <a>
                                        <img href="<?php echo(url_for(MOTDIT_PATH.'/single_page.php?articleID='.$informationCategorie['articleID'])); ?>" src="<?php echo(url_for(IMAGES_PATH.'/photosArticle/'.$informationCategorie['cheminImage'])) ?>" alt="Image de l'article" style="width: 200px; height: 200px">
                                    </a>
                                </div>
                                <div class="media-body">
                                    <div>
                                        <h6 style="margin: 0;">
                                            <a><?php echo($informationCategorie['nom']) ?></a>
                                        </h6>
                                        <h4 style="margin-bottom: 15px;">
                                            <a><b><?php echo($informationCategorie['titre']) ?></b> par <b><?php echo($informationCategorie['prenom'].' '.$informationCategorie['Nom auteur']) ?></b></a>
                                        </h4>
                                        <span>
                                            <i class="fa fa-calendar" style="font-size: 18px; padding-bottom: 10px"> Publié le : <?php echo($informationCategorie['dateParution']) ?></i>
                                        </span>
                                        <p style="padding-bottom: 11px;"><?php echo(substr($informationCategorie['texteArticle'], 0, 150).'...'); ?></p>
                                        <a id="<?php echo($informationCategorie['articleID']) ?>" href="<?php echo(url_for(MOTDIT_PATH.'/single_page.php?articleID='.$informationCategorie['articleID'])); ?>" class="btn_filtre">Lire ⇾</a>
                                    </div>
                                </div>
                            </article>
                        </li>
                    ...
                    <?php endforeach; ?>
                    <?php echo('<h4>'."Il y a présentement ".count($infosArticlesCategorie)." article(s) dans la catégorie : ".$informationCategorie['nom'].'</h4>') ?>
                <?php } else { ?>
                    <div>
                        <h1>Désolé il n'y a aucun article dans la catégorie sélectionnée</h1>
                    </div>
                <?php } ?>
